...
Scryfall supplies the majority of card data and the data skeleton. Gatherer supplies localization data that Scryfall lacks, keyed by multiverseId. MTGCH supplies unofficial localization text. MTGJSON supplies set-related data (still on the Scryfall skeleton) and currently does not cover card-related data. This division is provisional — it must be re-confirmed against each source's API before the import is finalized.
