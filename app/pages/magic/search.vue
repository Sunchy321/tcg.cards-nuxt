  ...
  <div>
    <Teleport to="#subheader-portal" defer>
      <div class="controller h-12 flex items-center px-4 shadow-md">
        <UIcon v-if="searching" name="tabler:refresh" class="animate-spin mr-2 text-white/80" />
...
import { explain as model } from '~/search/magic';
...
const cardLink = (card: CardPrintView) =>
  `/magic/card/${card.cardId}?set=${card.set}&number=${card.number}&lang=${card.lang}&part=${card.partIndex}`;
