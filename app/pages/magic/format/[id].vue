  ...
  <div class="pt-6">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <div v-if="!showTimeline" class="flex items-center gap-2 bg-white dark:bg-gray-900 rounded-lg px-2 py-1">
        <UButton icon="tabler:arrow-left-circle" variant="ghost" color="neutral" size="sm" @click="toPrevDate" />
        <input
          type="date"
        ...
        >
        <UButton icon="tabler:arrow-right-circle" variant="ghost" color="neutral" size="sm" @click="toNextDate" />
      </div>
    </div>
    ...
    <div v-if="showTimeline">
      ...
      <UCard v-for="n in nodes" :key="n.date" class="mb-4">
        <div class="flex items-center gap-2 mb-3">
          ...
          >
            <UIcon name="tabler:link" class="w-4 h-4" />
          </a>
        </div>
        ...
        <div v-if="n.sets.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 mb-3">
          ...
          >
            <UIcon
              :name="status === 'legal' ? 'tabler:plus' : 'tabler:minus'"
...
const route = useRoute('magic-format-id');
...
watch([format, formatList, showTimeline], () => {
  setParams([
    {
      id:       'format',
      type:     'select',
      items:    formatList.value.map(f => ({ value: f, label: i18n.t(`magic.format.${f}`) })),
      value:    format.value,
      onChange: (val: string) => router.push({ path: `/magic/format/${val}`, query: route.query }),
    },
    {
      id:       'timeline',
      type:     'switch',
      ...
      onChange: val => router.replace({ query: { ...route.query, timeline: val ? null : undefined } }),
    },
  ]);
}, { immediate: true });
